...
public class Day22 : IAdvent
{
    ...
    static readonly char[] Facing = new[] { '>', 'v', '<', '^' };
    static readonly V2[] MoveD = new V2[] { new (1, 0), new (0, 1), new (-1, 0), new (0, -1) };
    public void Run()
    {
        ...
        var startPoint = new V2(0, 0);
        while (maze[startPoint] != '.')
            startPoint += MoveD[0];
        int cubeSideSize = Math.Max(maxY, maxX) / 4;


        int answer1 = DoIt(maze, instructions, startPoint, movePosition: MovePart1);
        Advent.AssertAnswer1(answer1, expected: 65368, sampleExpected: 6032);


        int answer2 = DoIt(maze, instructions, startPoint, movePosition: MovePart2);
        Advent.AssertAnswer2(answer2, expected: 156166, sampleExpected: 5031);


        int DoIt(Dictionary<V2, char> maze, string[] instructions, V2 p, Func<V2, int, (V2 pos, int facing)> movePosition)
        {
            int facing = 0;
            ...
            foreach (var instr in instructions)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < moves; i++)
                    {
                        ...
                        (V2 newPos, int newFacing) = movePosition(p, facing);
                        if (newPos.Y < 0)
                            1.ToString();
                        if (maze[newPos] == '#')
                            break;
        ...
        (V2, int) MovePart1(V2 p, int facing)
        {
            V2 newPos = p;
            do
            {
        ...
        (V2, int) MovePart2(V2 p, int facing)
        {
            ...
            return FindNewPositionByMoveNearEdge(p, facing);
        }

        (V2 p, int facing) FindNewPositionByMoveNearEdge(V2 p, int facing)
        {
    ...
    public record Vector3(int X, int Y, int Z)
    {
        ...
        public Vector3 Reverse() => new(-X, -Y, -Z);
